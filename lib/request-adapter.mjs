...
/** Adapt content for Anthropic format (supports text, image, and tool_result blocks). */
...
/** Adapt content for OpenAI format (supports text, image_url, and tool results). */
function adaptContentForOpenAI(content) {
  ...
  if (Array.isArray(content)) {
    return content.map((c) => {
      ...
      // Convert Anthropic tool_result to OpenAI tool message format (handled at message level)
      if (c.type === 'tool_result') {
        return { type: 'text', text: JSON.stringify(c.content || c) };
      }
      return c;
    });
  }
