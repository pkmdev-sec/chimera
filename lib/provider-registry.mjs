...
export class ProviderRegistry {
  ...
  register(id, config) {
    if (!id || typeof id !== 'string') throw new Error('Provider id must be a non-empty string');
    if (!config || typeof config !== 'object') throw new Error('Provider config must be an object');
    const required = ['name', 'baseUrl', 'models'];
    for (const key of required) {
      if (!config[key]) throw new Error(`Provider config missing required field: ${key}`);
    }
